...
class Tank_auth
{
	 ...
	 * @return	array
	 */
	function create_user_client($username, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			if (!is_null($res = $this->ci->users->create_user_client($data, !$email_activation))) {
        ...
        function create_user_restaurant($username, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');
			...
			if (!is_null($res = $this->ci->users->create_user_restaurant($data, !$email_activation))) {
				$data['user_id'] = $res['user_id'];
				$data['password'] = $password;
				unset($data['last_ip']);
				return $data;
	 ...
	 */
        ...
        function update_profile_client($profile_data, $user_id)
	{
		return $this->ci->users->update_profile_client($profile_data, $user_id);
	}
	 ...
	 */
        ...
        function update_profile_restaurant($profile_data, $user_id)
	{
		return $this->ci->users->update_profile_restaurant($profile_data, $user_id);
	}
